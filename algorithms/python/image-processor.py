...
def parse_string_data(ocr):
    # get the row data from the OCR
    data_rows = get_data_rows(ocr)
    # create a new list to store gems 
    gem_list = []
    # for each row in the data, get the gem data
    for i in range(len(data_rows)):
        row = data_rows[i]
        gem_list.append(get_gem_data(i + 1, row))
    # create output and add the gem data to it
...
for page in range(1, IMAGE_COUNT + 1):
    ...
    # get the ocr data from tesseract
